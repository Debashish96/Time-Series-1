...
```{r, echo=T}
## Look at the count for different SKU values (use table function)
table(data$SKU)
```
...
```{r, echo=T}
data3 = data3[order(data3$Date, decreasing=F), ]
...
data3 = data3 %>% group_by(Date) %>% summarise("MinPrice" = min(Price))
...
```{r, echo=T}
minDate = min(data3$Date)
maxDate = max(data3$Date)
...
data3 = seq %>% full_join(data3, c("DateRange" = "Date"))
...
```{r, echo=T}
data3$MinPrice = (na.locf(data3$MinPrice, fromLast = FALSE) + 
                    na.locf(data3$MinPrice, fromLast = TRUE))/2

head(data3)
plot(data3$MinPrice, type = 'l')
...
data3$Year = as.numeric(format(data3$DateRange, format="%Y"))
data3$Month = as.numeric(format(data3$DateRange, format="%m"))


data3 = data3 %>% group_by(Year, Month) %>% summarise("MeanPrice" = mean(MinPrice))

print(data3)

data3 = data.frame(data3)

plot(data3$MeanPrice, type = 'l')
...
### ACF, PACF plots 
#LAG=44 means construct 44 lines
